...
export class NewOwnership extends Entity {
  ...
  save(): void {
    ...
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        "Cannot save NewOwnership entity with non-string ID. " +
          'Considering using .toHex() to convert the "id" to a string.'
      );
      store.set("NewOwnership", id.toString(), this);
    }
  }

  static load(id: string): NewOwnership | null {
    return changetype<NewOwnership | null>(store.get("NewOwnership", id));
  }
...
export class NewPendingOwnership extends Entity {
  ...
  save(): void {
    ...
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        "Cannot save NewPendingOwnership entity with non-string ID. " +
          'Considering using .toHex() to convert the "id" to a string.'
      );
      store.set("NewPendingOwnership", id.toString(), this);
    }
  }

  static load(id: string): NewPendingOwnership | null {
    return changetype<NewPendingOwnership | null>(
      store.get("NewPendingOwnership", id)
    );
  }
